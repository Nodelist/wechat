...
public class WeChatServiceImpl implements WeChatService, BaseAsyncService {
    ...
    @Override
    public void getAppList(JsonObject params, Handler<AsyncResult<JsonObject>> handler) {
    ...
    @Override
    public void deleteAppById(JsonObject params, Handler<AsyncResult<JsonArray>> handler) throws IllegalArgumentException {
    ...
    @Override
    public void addApp(JsonObject params, Handler<AsyncResult<JsonObject>> handler) {
        Future<JsonObject> future= Future.future();
    ...
    @Override
    public void editAppById(JsonObject params, Handler<AsyncResult<JsonObject>> handler) throws IllegalArgumentException {
        Future<JsonObject> future= Future.future();
        String id = params.getString("id");
        if (id.isEmpty()) {
            throw new IllegalArgumentException("app is not found");
    ...
    // 异步查询密钥
    public void getAppSecret(JsonObject params, Handler<AsyncResult<JsonObject>> handler) {
        Future<JsonObject> future= Future.future();
        ExampleMatcher exampleMatcher = ExampleMatcher.matching().withStringMatcher(ExampleMatcher.StringMatcher.CONTAINING);
        WeChat weChat = new WeChat();
        String appid = params.getString("appid");
    ...
    // 异步查询商户信息
    public void getMerchant(JsonObject params, Handler<AsyncResult<JsonObject>> handler) {
        Future<JsonObject> future= Future.future();
        ExampleMatcher exampleMatcher = ExampleMatcher.matching().withStringMatcher(ExampleMatcher.StringMatcher.CONTAINING);
        WechatMerchant wechatMerchant = new WechatMerchant();
        String appId = params.getString("appId");
        if (!StringUtils.isEmpty(appId)) {
            wechatMerchant.setPidApp(appId);
            exampleMatcher.withMatcher("appId", ExampleMatcher.GenericPropertyMatchers.startsWith());
        }
