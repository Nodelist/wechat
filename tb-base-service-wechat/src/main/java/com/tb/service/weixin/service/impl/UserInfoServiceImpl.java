...
//        String appsecret = weChatServiceImpl.getAppSecret(appid);   //调用通过appid获取密钥的方法
        JsonObject resultJson = new JsonObject();
        weChatServiceImpl.getAppSecret(paramter, res -> {
//            resultJson.put("data", new JsonObject(Json.encode(res.result())));
            resultJson.put("data",res.result().getValue("appSecret"));
        });
        String appsecret = resultJson.getString("data");
        String openid_Url = url.replace("APPID", appid).replace("APPSECRET", appsecret).replace("CODE", code);
        CloseableHttpResponse closeableHttpResponse = null;
